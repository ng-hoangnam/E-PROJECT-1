<?php
    ...
    if(isset($_POST['apply'])){
        var_dump($_FILES);
        $result = $conn->query("SELECT brands.BRANDNAME FROM brands WHERE BRANDID = '$id_edit'");
        $linkimg = mysqli_fetch_array($result);
        $brandimg = $linkimg['BRANDNAME'] . '/';
            ...
            if(in_array($file_actuaext , $allowed)){
                if($file_error === 0) {
                    if($file_size < 1000000){
                        ...
                        $fie_destination = '../assets/img/lightType/' . $brandimg . $file_namenew ;
        ...
        if(empty($productsaleprice)){
            $errors['productsaleprice'] = "Cannot empty";
        }
        ...
        if(empty($errors) && $file_error === 0){
            if(!empty($_FILES['file']['name'])) {
                $img_up = 'assets/img/lightType/'.$brandimg.'/'.$file_namenew;
            }
            $sql_pushedited = "UPDATE products SET PRODUCTNAME = '$productname' , DETAIL = '$productdetail' , QUANTITY = '$productquantity' , STATUS = '$productstatus' , BRANDID = '$productbrand' , PRICE = '$productprice' , SALEPRICE = '$productsaleprice' , SOURCE = '$img_up' WHERE PRODUCTID = '$id_edit' ";
